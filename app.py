...
def login_user():
    ...
    try:
        ...
        otp_ok, otp_error = save_and_email_otp(user.get('user_id'), user.get('email'), user.get('full_name'), purpose='Login')
        if not otp_ok:
...
def resend_otp():
    ...
    try:
        ...
        otp_ok, otp_error = save_and_email_otp(record.get('id'), record.get('email'), record.get('full_name'), purpose=purpose)
        if not otp_ok:
            return jsonify({"error": f"Unable to send OTP: {otp_error}"}), 502
